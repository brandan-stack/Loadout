...
export function SettingsPageClient({ initialSettings }: SettingsPageClientProps) {
  ...
  useEffect(() => {
    primeCachedData(TAB_DATA_CACHE_KEYS.settings, initialSettings);
  ...
  async function fetchSettings(showSpinner = false, force = false) {
    if (showSpinner) {
      setLoading(true);
    }
    ...
    try {
      if (force) {
        invalidateCachedData(TAB_DATA_CACHE_KEYS.settings);
      }

      const data = await fetchJsonWithCache<AppSettings>(TAB_DATA_CACHE_KEYS.settings, "/api/settings");
      setSettings(data);
    } catch (error) {
