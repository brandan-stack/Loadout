...
export function LocationsPageClient({ initialLocations }: LocationsPageClientProps) {
  ...
  useEffect(() => {
    primeCachedData(TAB_DATA_CACHE_KEYS.locationsFull, initialLocations);
  }, [initialLocations]);

  async function loadLocations(showSpinner = false, force = false) {
    if (showSpinner) {
      setLoading(true);
    }
    ...
    try {
      if (force) {
        invalidateCachedData(TAB_DATA_CACHE_KEYS.locationsFull);
      }

      const data = await fetchJsonWithCache<Location[]>(
        TAB_DATA_CACHE_KEYS.locationsFull,
        "/api/locations?includeStock=1"
      );
